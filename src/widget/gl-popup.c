...
enum gl_popup_obj_del_cb_type {
	GL_POPUP_OBJ_DEL_CB_TYPE_ITEM, GL_POPUP_OBJ_DEL_CB_TYPE_ALBUM
};
...
static void __gl_popup_del_resp_cb(void *data, Evas_Object *obj,
	...
	GL_CHECK(text); gl_dbg("Button: %s", text);
...
static void __gl_popup_resp(void *data, const char *text) {
	...
	GL_CHECK(text); gl_dbg("Button: %s, mode: %d", text, mode);
...
static void __gl_popup_local_mouseup_cb(void *data, Evas *e, Evas_Object *obj,
		void *ei) {
	gl_dbg("");
	GL_CHECK(data);
	GL_CHECK(ei);
...
static void __gl_popup_local_delete_cb(void *data, Evas *e, Evas_Object *obj,
		void *ei) {
	gl_dbg("Delete popup ---");
	GL_CHECK(data);
	gl_appdata *ad = (gl_appdata *) data;
...
static void __gl_popup_local_block_clicked_cb(void *data, Evas_Object *obj,
		void *ei) {
	GL_IF_DEL_OBJ(obj);
}
...
static Evas_Object *__gl_popup_add(Evas_Object *parent, const char *style,
		const char *title, const char *text, void *data) {
...
static void __gl_popup_genlist_realized(void *data, Evas_Object *obj, void *ei) {
	if (elm_genlist_first_item_get(obj) == ei) {
		gl_dbg("Emit signal");
		evas_object_smart_callback_call(obj, "popup,genlist,realized", data);
	}
}

static Evas_Object *__gl_popup_add_genlist(void *data, Evas_Object *parent,
		const char *style, Elm_Gen_Item_Text_Get_Cb text_get,
		Elm_Gen_Item_Content_Get_Cb content_get, void *append_func) {
	GL_CHECK_NULL(parent);
	...
	evas_object_event_callback_add(genlist, EVAS_CALLBACK_DEL,
			__gl_popup_genlist_delete_cb, (void *) gic);
...
static void __gl_popup_delete_cb(void *data, Evas *e, Evas_Object *obj,
	...
	GL_FREEIF(data); gl_dbg("Delete popup +++");
}
...
static void __gl_popup_thumb_longpressed_append(void *data, Evas_Object *gl,
	...
	GL_CHECK(gic);
	GL_CHECK(gl);
	GL_CHECK(data); gl_dbg("append items");
	gl_popup_menu_s *share_d = (gl_popup_menu_s *) data;
	GL_CHECK(share_d->ad);
	gl_appdata *ad = share_d->ad;
	...
	GL_CHECK(gitem);
	GL_CHECK(gitem->item); gl_dbg("item storage type: %d", gitem->item->storage_type);
	elm_genlist_item_append(gl, gic, (void *) GL_STR_ID_SHARE,
	NULL, ELM_GENLIST_ITEM_NONE, __gl_popup_menu_sel_cb, data);
...
static void __gl_popup_move_cb(void *data, Evas_Object *obj, void *ei) {
	...
	GL_CHECK(obj);
	gl_popup_menu_s *menu_d = NULL;
	menu_d = (gl_popup_menu_s *) evas_object_data_get(obj,
			GL_POPUP_OBJ_DATA_KEY);
	GL_CHECK(menu_d);
	GL_CHECK(menu_d->ad);
	gl_appdata *ad = menu_d->ad;
...
static void __gl_popup_move_append(void *data, Evas_Object *gl,
	...
	for (i = 0; i < all_count; i++) {
		...
		if (album->cluster->type == GL_STORE_T_MMC
			...
			NULL, ELM_GENLIST_ITEM_NONE, __gl_popup_move_cb,
					album->cluster->path);
		}
	}
}
...
static void __gl_popup_genlist_resize_job_cb(void *data) {
	...
	if (track) {
		...
		if (resize_d->item_height != h) {
			...
			GL_POPUP_GENLIST_ITEM_W * elm_config_scale_get(), min_h);
		}
	}
...
static void __gl_popup_genlist_realized_resize(void *data, Evas_Object *obj,
		void *ei) {
	GL_CHECK(data);
	gl_popup_resize_s *resize_d = (gl_popup_resize_s *) data;
	GL_IF_DEL_JOB(resize_d->job);
...
static int __gl_popup_add_resize_op(gl_appdata *ad, Evas_Object *box,
		Evas_Object *genlist) {
	...
	evas_object_smart_callback_add(genlist, "popup,genlist,realized",
			__gl_popup_genlist_realized_resize, (void *) resize_d);
	evas_object_data_set(genlist, GL_POPUP_GENLIST_DATA_KEY, (void *) resize_d);
...
static int __gl_popup_create_menu(void *data, const char *title,
	...
	Evas_Object *popup = __gl_popup_add(ad->maininfo.win, "content_no_vhpad",
			title, NULL, data);
	if (popup == NULL) {
		GL_FREE(menu_d);
		return -1;
	...
	genlist = __gl_popup_add_genlist(menu_d, box, "1text/popup",
			__gl_popup_menu_get_text, NULL, append_func);
...
static int __gl_popup_create_album_menu(void *data, const char *title,
	...
	Evas_Object *box = NULL;
	Evas_Object *genlist = NULL;
	Evas_Object *popup = __gl_popup_add(ad->maininfo.win, NULL, title, NULL,
			data);
	if (popup == NULL) {
		GL_FREE(menu_d);
		return -1;
...
int _gl_popup_create_del(void *data, int mode, void *sel_cb, void *del_cb,
	...
	GL_STR_ID_DELETE, popup_str, data);
	GL_CHECK_VAL(popup, -1);
	evas_object_event_callback_add(popup, EVAS_CALLBACK_DEL,
	...
	if (sel_cnt > 1) {
		evas_object_smart_callback_add(popup, "language,changed",
				__gl_popup_del_lang_changed, (void *) sel_cnt);
...
int gl_popup_create_popup(void *data, gl_popup_mode mode, char *desc) {
	...
	if (mode == GL_POPUP_ALBUM_MEMORY_FULL) {
		popup = __gl_popup_add(ad->maininfo.win, NULL, GL_DATA_SAVE_FAILED,
				desc, data);
	} else {
	...
	case GL_POPUP_ALBUM_RENAME_DUPLICATE:
	case GL_POPUP_ALBUM_RENAME_INVALID:
		btn1 = _gl_but_create_but_popup(popup, GL_STR_ID_NO, __gl_popup_resp_cb,
				data);
		elm_object_part_content_set(popup, "button1", btn1);
...
Evas_Object *_gl_popup_create_local(void *data, gl_popup_mode mode, char *desc) {
	...
	case GL_POPUP_NOBUT:
		...
		ad->popupinfo.del_timer = ecore_timer_add(GL_POPUP_TIMEOUT_3S,
				__gl_popup_local_timeout_cb, popup);
		evas_object_smart_callback_add(popup, "block,clicked",
				__gl_popup_local_block_clicked_cb, popup);
		break;
	case GL_POPUP_ONEBUT:
...
int _gl_popup_create_move(void *data, void *op_func) {
	gl_dbg("");
	GL_CHECK_VAL(data, -1);
	__gl_popup_create_album_menu(data, GL_STR_ID_MOVE, __gl_popup_move_append,
			op_func, GL_POPUP_MEDIA_MOVE);
...
int _gl_popup_create_copy(void *data, void *op_func) {
	gl_dbg("");
	GL_CHECK_VAL(data, -1);
	__gl_popup_create_album_menu(data, GL_STR_ID_COPY, __gl_popup_move_append,
			op_func, GL_POPUP_MEDIA_MOVE);
	_gl_popup_add_buttons(data, GL_STR_ID_NEW_ALBUM,
			__gl_popup_btn_new_album_cb);
	return 0;
...
int _gl_popup_create_move_with_append(void *data, void *op_func,
		void *append_func, const char *title) {
	gl_dbg("");
	GL_CHECK_VAL(data, -1);
...
int _gl_popup_create_longpressed_album_with_append(void *data, void *op_func,
		void *append_func, const char *title) {
	gl_dbg("");
	GL_CHECK_VAL(data, -1);
...
int _gl_popup_create_longpressed_thumb_with_append(void *data, void *op_func,
		void *append_func, const char *title) {
	gl_dbg("");
	GL_CHECK_VAL(data, -1);
	__gl_popup_create_menu(data, title, append_func, op_func,
...
int _gl_popup_create_longpressed_thumb(void *data, void *op_func,
		char *file_name) {
	gl_dbg("");
	GL_CHECK_VAL(data, -1);
	__gl_popup_create_menu(data, file_name, __gl_popup_thumb_longpressed_append,
			op_func, GL_POPUP_LONGPRESSED);
	_gl_popup_add_block_callback(data);
	return 0;
}
...
void _gl_list_viewas_popup_cb_add(Evas_Object *btn, Evas_Object *btn1, void *ad) {
	evas_object_smart_callback_add(btn, "clicked", _gl_time_view_selected_cb,
			ad);
	evas_object_smart_callback_add(btn1, "clicked", _gl_album_view_selected_cb,
			ad);
}

void _gl_list_sortby_popup_cb_add(Evas_Object *btn, Evas_Object *btn1, void *ad) {
	evas_object_smart_callback_add(btn, "clicked", _gl_desc_sort_selected_cb,
			ad);
	evas_object_smart_callback_add(btn1, "clicked", _gl_asc_sort_selected_cb,
			ad);
}

void _gl_list_pop_create(void *data, Evas_Object *obj, void *ei, char *title,
		char *first_text, char *second_text, int state_index)
{
	gl_dbg("ENTRY");
	GL_CHECK(data);
	gl_appdata *ad = (gl_appdata *) data;
	...
	Edje_Message_Int *msg = (Edje_Message_Int *) malloc(
			sizeof(Edje_Message_Int) + sizeof(int));
	GL_CHECK(msg);
	if ((w == GL_STR_SCREEN_WIDTH_WVGA && h == GL_STR_SCREEN_HEIGHT_WVGA)
		...
		edje_object_message_send(elm_layout_edje_get(ly), EDJE_MESSAGE_INT, 2,
				msg);
	} else if ((w == GL_STR_SCREEN_WIDTH_QHD && h == GL_STR_SCREEN_HEIGHT_QHD)
			|| (w == GL_STR_SCREEN_HEIGHT_QHD && h == GL_STR_SCREEN_WIDTH_QHD)) {
		msg->val = GL_POPUP_GENLIST_HEIGHT_QHD;
		edje_object_message_send(elm_layout_edje_get(ly), EDJE_MESSAGE_INT, 2,
				msg);
	} else if ((w == GL_STR_SCREEN_WIDTH_HD && h == GL_STR_SCREEN_HEIGHT_HD)
...
void _gl_genlist_item_apend(void *data, Evas_Object *gl,
	...
	for (i = 0; i < all_count; i++) {
		...
		if (album->cluster->type == GL_STORE_T_MMC
			...
			NULL, ELM_GENLIST_ITEM_NONE, __gl_popup_move_copy_sel_cb,
					album->cluster->path);
		}
	}
}
...
int _gl_popup_create_copy_move(void *data, void *sel_cb, void *cb_data) {
	...
	elm_object_part_content_set(popup, "button1", btn1);
	elm_object_part_content_set(popup, "button2", btn2);
	evas_object_smart_callback_add(btn1, "clicked", __gl_thumbs_edit_cancel_cb,
			ad);
	...
	Evas_Object *genlist = __gl_popup_add_genlist(menu_d, popup, "default",
			...
			__gl_popup_menu_get_genlist_content, _gl_genlist_item_apend);
	Evas_Object *ly = elm_layout_add(popup);
	elm_layout_file_set(ly, GL_EDJ_FILE, "list_popup_swallow_ly");
